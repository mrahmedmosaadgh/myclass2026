<template>
    ...
    <q-card class="main-container-card shadow-lg rounded-2xl overflow-hidden">
      <!-- <q-card-section class="bg-gradient-to-r from-blue-500 to-purple-600 text-white">
        <div class="text-2xl font-bold flex items-center gap-3">
          ...
          🌟 {{ t('common.settings') || 'Super Star Classroom' }} 🌟
        </div>
        <div class="text-blue-100 mt-1">✨ Where every student shines and learns! ✨</div>
      </q-card-section> -->
        ...
        <div class="control-panel">
          <div class="flex flex-col gap-4">
            <div class="flex  flex-row justify-center gap-4">
              ...
              <!-- <q-btn color="primary" icon="people" label="Load Teachers"
             ...
             -->
                ...
                <q-tooltip
                  ...
                  <q-icon :name="!fetchData.my_sel_classroom_students?.length ? 'warning' : 'emoji_events'" size="xs"
                    class="mr-1" />
                  {{ !fetchData.my_sel_classroom_students?.length ? 'Select a classroom first to view leaderboard' : 'View top 5 students with highest star points!' }}
                </q-tooltip>
              </q-btn>
              <q-btn color="grey-7" icon="settings" label="Settings" @click="showSettingsDialog = true"
                class="shadow-sm rounded-lg settings-main-btn" size="md">
                <q-tooltip class="bg-grey-8 text-white" anchor="top middle" self="bottom middle">
                  <q-icon name="tune" size="xs" class="mr-1" />
                  Open classroom settings - manage avatar editing and attendance
                </q-tooltip>
              </q-btn>
            </div>
            <!-- Classroom Selection -->
  <!-- <div class="w-full flex justify-center  ">
            ...
            <div class="mb-6 bg-red-400 w-fit" v-if="fetchData.my_classes_with_students?.length">
              <div class="flex items-center justify-between mb-3">
                <h3 class="text-lg font-semibold text-gray-700">Select Classroom</h3>
                <q-btn v-if="fetchData.my_sel_classroom_students?.length" color="info" icon="assessment"
                  label="Load All Student Summaries" @click="loadAllStudentSummaries" class="shadow-sm rounded-lg"
                  size="md" :loading="loadingSummaries">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                <q-card v-for="classroom in fetchData.my_classes_with_students" :key="classroom.id"
                  class="classroom-card cursor-pointer transition-all duration-200"
                  :class="{ 'selected-classroom': link === classroom.id }"
                  @click="handleClassroomSelection(classroom.id, classroom.students)" flat bordered>
                  <q-card-section class="flex items-center gap-3 py-3">
                    <q-icon name="class" color="primary" size="md" />
                    <div class="font-medium text-gray-800">
                      {{ classroom.classroom_name }}
                    </div>
                    <q-badge v-if="classroom.students?.length" color="primary" rounded class="ml-auto">
              ...
              <!-- Period / Semester Selection -->
              <div class="mt-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
                <q-select filled v-model="selSemester" :options="semesterOptions" option-value="value" option-label="label" emit-value map-options label="Semester" class="rounded-lg" />

                <q-select filled v-model="selWeek" :options="weekOptions" option-value="value" option-label="label" emit-value map-options label="Week" class="rounded-lg" />

                <q-select filled v-model="selDay" :options="dayOptions" option-value="value" option-label="label" emit-value map-options label="Day" class="rounded-lg" />

                <q-select filled v-model="selPeriodNumber" :options="periodOptions" option-value="value" option-label="label" emit-value map-options label="Period Number" class="rounded-lg" />
                ...
                <q-btn v-if="fetchData.my_sel_classroom_students?.length" color="info" icon="assessment"
                  label="Load  Summaries" @click="loadAllStudentSummaries" class="shadow-sm rounded-lg"
                  size="md" :loading="loadingSummaries">
                  <q-tooltip class="bg-info text-white" anchor="top middle" self="bottom middle">
                    <q-icon name="assessment" size="xs" class="mr-1" />
                    Load behavior summaries for all students in this classroom
                                       ...
                                       <!-- <q-btn     size="sm" color="blue" text-color="dark"  
                        
                                
                                       @click.stop.prevent="loadAllStudentSummaries"
                                       class="shadow-lg avatar-btn-enabled">load 
                                      
                                    code:   {{ period_code }}    N:{{ fetchData.my_sel_classroom_students?.length }}
                                      </q-btn> -->
                      
                       
                <!-- Period code selection -->
                <!-- <q-select filled v-model="period_code" :options="periodCodeOptions" use-input input-debounce="300" multiple=false hide-selected emit-value map-options label="Period Code" class="rounded-lg" /> -->
              </div>
  ...
  <div class="flex flex-wrap justify-center gap-4 p-4 font-kid">
    
    <q-card
      v-for="student in fetchData.my_sel_classroom_students"
      :key="student.id"
      :class="[getStudentCardClass(student.id), { 'selected-card': link === student.id }]"
      @click="!shouldDisableBehaviorButtons(student.id) ? link = student.id : null"
      class="rounded-xl w-48  shadow-md transition-transform duration-300 hover:scale-105 bg-gradient-to-br from-blue-50 to-white"
      flat
    > 
      <!-- Avatar Section -->
      <div class="relative   rounded-t-xl  ">
        <img :src="student.avatar" :ratio="1" class="w-32 h-32  absolute rounded-full -top-16  -right-5"   /> 
        <!-- <q-img :src="student.avatar" :ratio="1" class="w-4 h-4  absolute rounded-full -top-10 left-0"  fit="cover">  -->
          <!-- Overlay buttons on hover -->
          <div
            class="absolute inset-0 bg-opacity-0 hover:bg-opacity-30 transition-all duration-300 flex items-center justify-center opacity-0 hover:opacity-100 avatar-overlay-enabled"
            v-show="shouldShowAvatarButtons && !shouldDisableBehaviorButtons(student.id)"
          >
            <div class="flex gap-2 scale-125">
              <q-btn
                round dense size="sm" color="white" text-color="dark" icon="photo_camera"
                @click.stop.prevent="openCamera(student)"
                class="shadow-lg avatar-btn-enabled"
              >
                <q-tooltip class="bg-primary text-white text-sm rounded-lg px-2 py-1">
                  📷 Take a photo!
                </q-tooltip>
              </q-btn>
            </div>
          ...
          <div
            v-show="!shouldShowAvatarButtons || shouldDisableBehaviorButtons(student.id)"
            class="absolute inset-0 bg-opacity-0 hover:bg-opacity-20 hover:bg-gray-500 transition-all duration-300 flex items-center justify-center opacity-0 hover:opacity-100 avatar-overlay-disabled"
          >
            <div class="flex gap-2 scale-125">
              <q-btn round dense size="sm" color="grey-5" text-color="grey-7" icon="photo_camera" disabled>
                <q-tooltip class="bg-grey-8 text-white text-sm rounded-lg px-2 py-1">
                  😴 {{ shouldDisableBehaviorButtons(student.id) ? 'Oops! They\'re not here today' : 'Camera is off' }}
                </q-tooltip>
              </q-btn>
            </div>
          </div>
      ...
      <!-- Student Info -->
      <q-card-section class="text-center py-3 px-4">
        <div class="  font-bold text-blue-800 mb-1">
          
           <NeonText class="scale-50" :text="parseName(student.name).firstName" colorA="blue" colorB="red" />
          {{ parseName(student.name).firstName }}
        </div>
        <div class="text-sm text-gray-600 mb-1" v-if="parseName(student.name).lastName">
          {{ parseName(student.name).lastName }}
        </div>
        <div class="text-xs text-gray-500" v-if="parseName(student.name).secondName">
          {{ parseName(student.name).secondName }}
        </div>
      </q-card-section>

      <!-- Actions & Stats -->
      <q-card-actions class="justify-between items-center px-4 py-3 bg-blue-50">
        <q-btn
          round dense
          :color="shouldDisableBehaviorButtons(student.id) ? 'grey-5' : 'primary'"
          icon="star"
          @click.stop.prevent="!shouldDisableBehaviorButtons(student.id) ? openBehaviorDialog(student.id, student.name) : null"
          :disable="shouldDisableBehaviorButtons(student.id)"
          class="shadow-sm behavior-btn"
        >
          <q-tooltip class="text-sm rounded-lg px-2 py-1"
            :class="shouldDisableBehaviorButtons(student.id) ? 'bg-grey-8 text-white' : 'bg-primary text-white'">
            🌟 {{ shouldDisableBehaviorButtons(student.id) ? 'Not available today' : 'Give a star!' }}
          </q-tooltip>
        </q-btn>

        <!-- Score Display -->
        <div class="flex flex-col items-center text-sm">
          <div class="text-lg font-bold text-primary">🎯 {{ studentSummaries[student.id]?.total || 0 }}</div>
          <div class="text-green-600" v-if="studentSummaries[student.id]?.positive > 0">
            🟢 +{{ studentSummaries[student.id]?.positive }}
          </div>
          <div class="text-red-500" v-if="studentSummaries[student.id]?.negative < 0">
            🔴 {{ studentSummaries[student.id]?.negative }}
          </div>
        </div>
      </q-card-actions>
    </q-card>
            ...
            <!-- Selection Controls -->
            <div class="selection-controls mt-6 space-y-4">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
     

                <!-- Classes Selection -->
                <div v-if="fetchData.classes">
                  <q-select filled v-model="sel.classes" :options="fetchData.classes" option-value="id"
                    option-label="name" multiple emit-value map-options label="Select Classes" use-chips
                    hint="You can select multiple classes" class="rounded-lg" />
                </div>
              </div>
              ...
              <!-- <div class="mt-4">
                <div class="flex items-center gap-4 mb-3">
                  <q-btn color="accent" icon="subject" label="Load Subjects"
                    @click="fetchDataGet('all_subjects', 'subjects')" class="shadow-sm rounded-lg" size="sm" />
                </div>

                <q-select v-if="fetchData.subjects" filled v-model="sel.subjects" :options="fetchData.subjects"
                  option-value="id" option-label="name" multiple emit-value map-options label="Select Subjects"
                  use-chips hint="You can select multiple subjects" class="rounded-lg" />
              </div> -->
    ...
    <q-dialog v-model="showBehaviorDialog">
      <q-card class="q-pa-md" style="min-width: 500px; max-width: 800px;">
        ...
        <q-card-section>
          ...
          <BehaviorManager :student-id="sel_student.id" :student-name="sel_student.f_l_name" :period-code="period_code"
            :behaviors="behaviors" :summary="studentSummaries[sel_student.id] || { positive: 0, negative: 0, total: 0 }"
            @recorded="onBehaviorRecorded" @close="showBehaviorDialog = false" />
        </q-card-section>
      </q-card>
    </q-dialog>
  ...
  <CameraCapture
      ...
      <!-- <q-card style="min-width:300px; max-width:700px;">
        <q-card-section>
          <div class="text-h6">Capture Avatar</div>
          <video ref="videoRef" autoplay playsinline
            style="width:100%;height:auto;border-radius:6px;background:#000"></video>
          <canvas ref="canvasRef" style="display:none"></canvas>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat label="Cancel" color="grey" @click="closeCamera" />
    ...
    <!-- Camera dialog using CameraCapture.vue -->
    <!-- <q-card v-model="showCameraDialog"> 
       <CameraCapture
          ...
          <div v-else class="leaderboard-list">
              ...
              <div class="podium">
                ...
                <div class="podium-position second-place" v-if="leaderboardData[1]">
                  <div class="podium-student">
                    ...
                    <h3 class="student-name">{{ parseName(leaderboardData[1].name).firstName }}</h3>
                ...
                <div class="podium-position first-place" v-if="leaderboardData[0]">
                  <div class="podium-student">
                    ...
                    <h3 class="student-name champion">{{ parseName(leaderboardData[0].name).firstName }}</h3>
                ...
                <div class="podium-position third-place" v-if="leaderboardData[2]">
                  <div class="podium-student">
                    ...
                    <h3 class="student-name">{{ parseName(leaderboardData[2].name).firstName }}</h3>
            ...
            <div class="remaining-students" v-if="leaderboardData.length > 3">
              <div v-for="(student, index) in leaderboardData.slice(3, 5)" :key="student.id" class="student-row">
                ...
                <div class="row-info">
                  <h4 class="row-name">{{ parseName(student.name).firstName }} {{ parseName(student.name).lastName }}
                  </h4>
            ...
            <div class="recognition-section" v-if="leaderboardData.length > 0">
              ...
              <div class="recognition-cards">
                <div class="recognition-card" v-if="leaderboardData[0]">
                  ...
                  <div class="card-name">{{ parseName(leaderboardData[0].name).firstName }}</div>
                </div>
                <div class="recognition-card" v-if="leaderboardData[1]">
                  <div class="card-icon">🌟</div>
                  <div class="card-title">Amazing Achiever!</div>
                  <div class="card-name">{{ parseName(leaderboardData[1].name).firstName }}</div>
                </div>
                <div class="recognition-card" v-if="leaderboardData[2]">
                  <div class="card-icon">⭐</div>
                  <div class="card-title">Fantastic Friend!</div>
                  <div class="card-name">{{ parseName(leaderboardData[2].name).firstName }}</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <q-dialog v-model="showSettingsDialog">
      <q-card style="min-width: 600px; max-width: 800px;">
        ...
        <q-card-section>
          ...
          <div class="settings-toggle-container enhanced-toggle-container">
            <q-toggle v-model="settings.avatarEditingEnabled" label="Enable Avatar Editing" color="primary" size="lg"
              class="mb-2 settings-toggle enhanced-settings-toggle"
              :class="{ 'toggle-enabled': settings.avatarEditingEnabled, 'toggle-disabled': !settings.avatarEditingEnabled }">
              <q-tooltip class="bg-grey-8 text-white tooltip-enhanced" anchor="top middle" self="bottom middle"
                :class="{ 'tooltip-success': settings.avatarEditingEnabled, 'tooltip-warning': !settings.avatarEditingEnabled }">
                <q-icon :name="settings.avatarEditingEnabled ? 'photo_camera' : 'photo_camera_off'" size="xs"
                  class="mr-1" />
                {{ settings.avatarEditingEnabled ? 'Avatar editing is currently enabled - students can upload/capture photos' : 'Avatar editing is currently disabled - students cannot change photos' }}
              </q-tooltip>
            </q-toggle>
            <div class="toggle-status-indicator enhanced-status-indicator">
              <q-icon :name="settings.avatarEditingEnabled ? 'check_circle' : 'cancel'"
                :color="settings.avatarEditingEnabled ? 'positive' : 'negative'" size="sm" class="status-icon"
                :class="{ 'icon-pulse': settings.avatarEditingEnabled }">
                <q-tooltip :class="settings.avatarEditingEnabled ? 'bg-positive text-white' : 'bg-negative text-white'"
                  anchor="top middle" self="bottom middle">
                  <q-icon :name="settings.avatarEditingEnabled ? 'thumb_up' : 'thumb_down'" size="xs" class="mr-1" />
                  Avatar editing is {{ settings.avatarEditingEnabled ? 'active' : 'inactive' }}
                </q-tooltip>
              </q-icon>
              <span class="text-sm font-medium transition-all duration-300"
                :class="settings.avatarEditingEnabled ? 'text-positive' : 'text-negative'">
                {{ settings.avatarEditingEnabled ? 'Enabled' : 'Disabled' }}
              </span>
              <!-- Visual state indicator -->
              <div class="ml-2 w-2 h-2 rounded-full transition-all duration-300"
                :class="settings.avatarEditingEnabled ? 'bg-positive shadow-green-glow animate-pulse' : 'bg-negative shadow-red-glow'">
              </div>
            </div>
          </div>
          <div class="text-caption text-grey-6 ml-8 mt-2">
              ...
              <div class="flex items-center gap-3">
                  ...
                  <div class="absolute -top-1 -right-1 attendance-status-badge">
                    <q-icon :name="isStudentPresent(student.id) ? 'check_circle' : 'cancel'"
                      ...
                      <q-tooltip
                        ...
                        {{ parseName(student.name).firstName }} is {{ isStudentPresent(student.id) ? 'present' : 'absent' }}
                      </q-tooltip>
                    </q-icon>
                  </div>
                ...
                <div class="student-info">
                  <div class="font-medium transition-all duration-300"
                    ...
                    {{ parseName(student.name).firstName }} {{ parseName(student.name).lastName }}
                  ...
                  <div class="text-sm transition-all duration-300"
                    ...
                    v-if="parseName(student.name).secondName">
                    {{ parseName(student.name).secondName }}
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-3">
                <q-chip :color="isStudentPresent(student.id) ? 'positive' : 'negative'" :text-color="'white'" size="sm"
                  ...
                  <q-tooltip :class="isStudentPresent(student.id) ? 'bg-positive text-white' : 'bg-negative text-white'"
                    ...
                    {{ parseName(student.name).firstName }} is currently marked as {{ isStudentPresent(student.id) ? 'present' : 'absent' }}
                  </q-tooltip>
                </q-chip>
                <q-toggle :model-value="isStudentPresent(student.id)"
                  ...
                  <q-tooltip class="bg-grey-8 text-white tooltip-enhanced" anchor="top middle" self="bottom middle">
                    ...
                    Click to mark {{ parseName(student.name).firstName }} as {{ isStudentPresent(student.id) ? 'absent' : 'present' }}
                  </q-tooltip>
                </q-toggle>
              </div>
            </div>
            ...
            <div class="mt-2">
              <q-linear-progress
                ...
                <q-tooltip class="bg-grey-8 text-white">
                  {{Math.round((fetchData.my_sel_classroom_students?.length ? (fetchData.my_sel_classroom_students.filter(s => isStudentPresent(s.id)).length / fetchData.my_sel_classroom_students.length) : 0) * 100) }}% attendance rate
                </q-tooltip>
              </q-linear-progress>
            </div>
          </div>
...
// Selection persistence (semester/week/day/period/classroom/period_code)
...
function loadLastSelection() {
  try {
    ...
    if (!raw) {
      // No saved selection: clear all selection refs
    ...
    // On error, clear all selection refs
    selSemester.value = null
    selWeek.value = null
    selDay.value = null
    selPeriodNumber.value = null
...
const placeholderAvatar = (name) => {
  // return data URI placeholder with initials (simple) or fallback path
  const initials = (name || '').split(' ').map(s => s[0] || '').slice(0, 2).join('').toUpperCase()
  return `https://via.placeholder.com/96x96.png?text=${encodeURIComponent(initials || 'NA')}`
}
...
const loadLeaderboard = async () => {
  ...
  try {
    ...
    }).filter(entry => entry.total > 0) // Only show students with positive points
...
const handleClassroomSelection = (classroomId, students) => {
  ...
  fetchData.value.my_sel_classroom_students = students || []
...
const toggleStudentAttendance = (studentId) => {
  try {
    ...
    const studentName = student ? parseName(student.name).firstName : `Student ${studentId}`
