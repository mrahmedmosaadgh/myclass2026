<template>
  <div class="p-6 space-y-6 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
   ...
   <!-- Attendance Stats with Session Info -->
    <AttendanceStats 
      ...
      :class-name="classrooms.find(c => c.classroom_id === selectedClassroomId)?.classroom_name"
      :period-code="periodCode"
      :session-date="selectedDate"
      :period-number="selectedPeriodNumber"
      @setup="showSetupDialog = true"
    />
        ...
        <q-tab-panel name="positive">
          <div class=" ">
            ...
            <div class="flex flex-col md:flex-row gap-6 mt-6">
              ...
              <!-- Right Column: Student Grid with Tabs -->
              <div class="flex-1 min-w-0">
                
                <!-- Group Tabs / Filter Bar -->
                <div v-if="organizedStudents.length > 1" class="mb-4 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden sticky top-4 z-20">
                  <div class="flex overflow-x-auto hide-scrollbar p-2 gap-2">
                    <button
                      v-for="(group, index) in organizedStudents"
                      :key="index"
                      class="flex items-center gap-2 px-3 py-2 rounded-lg border transition-all whitespace-nowrap min-w-fit select-none"
                      :class="(activeGroupFilter ? activeGroupFilter === group.name : index === 0) 
                        ? 'bg-indigo-50 border-indigo-500 ring-1 ring-indigo-500 shadow-sm' 
                        : 'bg-white border-gray-200 hover:border-indigo-300 hover:bg-indigo-50'"
                      @click="activeGroupFilter = group.name"
                    >
                      <!-- Select All Checkbox -->
                      <div @click.stop class="flex items-center">
                        <q-checkbox
                          ...
                          class="mr-1"
                        >
                          <q-tooltip>Select All {{ group.name || 'Students' }}</q-tooltip>
                        </q-checkbox>
                      </div>

                      <!-- Group Name & Count -->
                      <div class="flex flex-col items-start leading-none text-left">
                        <span class="font-bold text-sm" :class="(activeGroupFilter ? activeGroupFilter === group.name : index === 0) ? 'text-indigo-900' : 'text-gray-700'">
                           <span v-if="group.name && group.name !== 'Unassigned'" class="text-xs opacity-70 mr-1">{{ index + 1 }}.</span>
                           {{ group.name || 'All Students' }}
                        </span>
                        <span class="text-[10px] text-gray-500">{{ group.students.length }} students</span>
                      </div>
                    </button>
                  </div>
                </div>
                ...
                <div class="min-h-[200px] relative">
                  <template v-for="(group, index) in organizedStudents" :key="index">
                    <div v-show="(activeGroupFilter ? activeGroupFilter === group.name : index === 0) || organizedStudents.length === 1">
                      
                      <!-- Student Cards -->
                      <div class="flex flex-wrap justify-center gap-4 py-2">
                        <div v-if="group.students.length === 0" class="w-full text-center py-12 text-gray-400">
                          <q-icon name="groups" size="3rem" class="mb-2 opacity-50" />
...
import TopLeaderboardTable from './reward_sys_comp/TopLeaderboardTable.vue'
import AttendanceStats from './reward_sys_comp/AttendanceStats.vue'
import card2 from './final/card2.vue'; // Adjust path as needed
import card3 from './final/card3.vue'; // Adjust path as needed
import UnifiedStudentCard from './reward_sys_comp/UnifiedStudentCard.vue'
...
// function isGroupSelected(group) { declared below


function isGroupSelected(group) {
  if (!group || !group.students.length) return false
  // Check if ALL present students in group are selected
  const presentStudents = group.students.filter(s => studentAttendance.value[s.id])
  if (presentStudents.length === 0) return false
  
  return presentStudents.every(s => selectedIds.value.includes(s.id))
}

function toggleGroupSelection(group, val) {
  if (!group) return
  ...
  if (val) {
    // Select all present students in group
    // Add IDs that are not already in selectedIds
    ...
    // Deselect all students in group
    selectedIds.value = selectedIds.value.filter(id => !presentIds.includes(id))
  }
}
...
// Watch organizedStudents to reset filter if needed
watch(organizedStudents, (newVal) => {
  // If current filter is invalid, reset to first group
  if (activeGroupFilter.value && !newVal.find(g => g.name === activeGroupFilter.value)) {
    activeGroupFilter.value = newVal[0]?.name || null
  }
})
