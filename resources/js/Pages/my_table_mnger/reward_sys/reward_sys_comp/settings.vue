<template>
  <div class="p-6 space-y-6">
        <!-- Student Info -->ani_class:{{ ani_class }}
        ...
        <div class="control-panel">
          <div class="flex flex-col gap-4">
            <div class="flex flex-row justify-center gap-4">
              <!-- <StudentViewToggle v-model:view="currentView" class="mb-4" /> -->
            ...
            <div class="mt-4">
              <!-- Cards View -->
              <!-- <div v-if="currentView === 'card'" class="row q-col-gutter-md">
                <div v-for="student in fetchData.my_sel_classroom_students" :key="student.id"
                  class="col-12 col-md-6 col-lg-4">
                  <StudentCard :student="student">
                    <template #actions>
                      <div class="flex gap-2">
                        <q-btn flat round color="primary" icon="edit" size="sm" @click="handleStudentAction(student)" />
                        <q-btn flat round color="amber" icon="star" size="sm" @click="awardPoints(student)" />
                      </div>
                    </template>
                  </StudentCard>
                </div>
              ...
              <!-- Table View -->
              <!-- <div v-else>
                <q-table :rows="fetchData.my_sel_classroom_students" :columns="studentColumns" row-key="id"
                  :rows-per-page-options="[10, 20, 50, 100]" :filter="filter" v-model:pagination="pagination"
                  ...
                  <template v-slot:body="props">
                    <q-tr :props="props" :class="{ 'cursor-not-allowed opacity-60': props.row.attend !== 1 }">
                      <q-td v-for="col in props.cols" :key="col.name" :props="props">
                        {{ props.row[col.field] }}
                      </q-td>
                      <q-td auto-width>
                        <div v-if="props.row.attend === 1" class="flex gap-2">
                          <q-btn flat round color="primary" icon="edit" size="sm"
                            @click="handleStudentAction(props.row)" />
                          <q-btn flat round color="amber" icon="star" size="sm" @click="awardPoints(props.row)" />
                        </div>
            ...
            <div class="mt-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
              <q-select filled v-model="selSemester" :options="semesterOptions" option-value="value"
                option-label="label" emit-value map-options label="Semester" class="rounded-lg" />

              <q-select filled v-model="selWeek" :options="weekOptions" option-value="value" option-label="label"
                emit-value map-options label="Week" class="rounded-lg" />

              <q-select filled v-model="selDay" :options="dayOptions" option-value="value" option-label="label"
                emit-value map-options label="Day" class="rounded-lg" />

              <q-select filled v-model="selPeriodNumber" :options="periodOptions" option-value="value"
                option-label="label" emit-value map-options label="Period Number" class="rounded-lg" />
              ...
              <q-select v-if="fetchData.my_classes_with_students"
                ...
                option-value="value" option-label="label" emit-value map-options label="Classroom" class="rounded-lg" />
            ...
            <div class="flex flex-wrap justify-center gap-2 p-2 font-kid">
  ...
  hover:z-50 hover:shadow-2xl hover:w-96  "
        flat
      >         
        <!-- Avatar Section -->
        <div class="relative w-full bg-green-500  justify-center  ">
          ...
          <div class="relative  "> 
            ...
            <transition name="fade">
              <div
                v-if="!shouldShowAvatarButtons || shouldDisableBehaviorButtons(student.id)"
                class="absolute inset-0 flex items-center justify-center bg-gray-500 bg-opacity-20 rounded-full transition-all duration-300"
              >
                <q-btn
                  round
                  dense
                ...
                >
                  <q-tooltip class="bg-grey-8 text-white text-sm rounded-lg px-2 py-1">
                    😴
                    {{
                      shouldDisableBehaviorButtons(student.id)
                        ? "Oops! They're not here today"
                        : "Camera is off"
                    }}
                  </q-tooltip>
                </q-btn>
        ...
        class="text-left mt-6 pb-2 overflow-hidden ">
        <div class="p-0 scale-75">

          <NeonText 
          ani_class="neon-text-on-hover "
          class="block   font-extrabold mb-1 neon-text-on-hover "
          :text="parseName(student.name).firstName"
          colorA="blue"
          colorB="red"
          />
        </div>
          <div
            ...
            v-if="parseName(student.name).lastName"
          >
        ...
        <!-- Actions & Stats -->
        <q-card-actions
          class="flex justify-between items-center px-5 py-3 bg-gradient-to-r from-blue-100 to-blue-50 rounded-b-2xl"
        >
          <!-- Behavior Button -->
          <q-btn
            round
          ...
          <div class="flex flex-col items-center text-sm leading-tight">
            <div class="text-lg font-bold text-primary">
              🎯 {{ studentSummaries[student.id]?.total || 0 }}
            </div>
            <transition name="fade">
              <div
                v-if="studentSummaries[student.id]?.positive > 0"
            ...
            <transition name="fade">
              <div
                v-if="studentSummaries[student.id]?.negative < 0"
                class="text-red-500"
              >
                  ...
                  <div class="absolute -top-1 -right-1 attendance-status-badge">
                    <q-icon :name="isStudentPresent(student.id) ? 'check_circle' : 'cancel'"
                      ...
                      <q-tooltip
                        ...
                        {{ parseName(student.name).firstName }} is {{ isStudentPresent(student.id) ? 'present' :
                          'absent' }}
                      </q-tooltip>
                    </q-icon>
                  </div>
              ...
              <div class="flex items-center gap-3">
                <q-chip :color="isStudentPresent(student.id) ? 'positive' : 'negative'" :text-color="'white'" size="sm"
                  ...
                  <q-tooltip :class="isStudentPresent(student.id) ? 'bg-positive text-white' : 'bg-negative text-white'"
                    ...
                    {{ parseName(student.name).firstName }} is currently marked as {{ isStudentPresent(student.id) ?
                      'present' : 'absent' }}
                  </q-tooltip>
                </q-chip>
                <q-toggle :model-value="isStudentPresent(student.id)"
                  ...
                  <q-tooltip class="bg-grey-8 text-white tooltip-enhanced" anchor="top middle" self="bottom middle">
                    ...
                    Click to mark {{ parseName(student.name).firstName }} as {{ isStudentPresent(student.id) ? 'absent'
                      :
                      'present' }}
                  </q-tooltip>
                </q-toggle>
...
import StudentViewToggle from '@/Components/Students/StudentViewToggle.vue'
import StudentCard from '@/Components/Students/StudentCard.vue'
// import ClassroomRecordsPage from './classroom_records/classroom_records_components/ClassroomRecordsPage.vue'
