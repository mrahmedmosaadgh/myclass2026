...
## Phase 1 — Backend (Fast Session Load)
...
- [ ]  Create migration: `create_cr_sessions_table`

  - Columns: `school_id`, `year_id`, `teacher_id`, `classroom_id`, `subject_id`, `date`, `day_number`, `period_number`, `period_code (VARCHAR)`, `status (enum: draft|active|locked)`, timestamps
  - Unique constraint: `(school_id, year_id, classroom_id, subject_id, teacher_id, date, period_code)`
- [ ]  Create migration: `create_cr_student_periods_table`

  - Columns: `school_id`, `year_id`, `student_id`, `session_id (nullable FK)`, `date`, `period_code`, `attendance_status (enum: present|absent|late|left_early)`, `attendance_score (0..5)`, `attendance_note (nullable)`, `total_score (0..20)`, `locked (boolean, default false)`, timestamps
  - Unique constraint: `(school_id, year_id, date, period_code, student_id)`
- [ ]  Create migration: `create_cr_category_mappings_table`

  - Columns: `school_id`, `key (VARCHAR)`, `label`, `type`, `max_value`, `passing_value`, `default_value`, `sort_order`, `active (boolean)`, timestamps
  - Unique constraint: `(school_id, key)`
- [ ]  Create migration: `create_cr_scores_table`

  - Columns: `student_period_id (FK)`, `mapping_id (FK)`, `numeric_value (nullable)`, `text_value (nullable)`, `json_value (nullable)`, timestamps
  - Unique constraint: `(student_period_id, mapping_id)`

---

### 1.2 — Seed Default Category Mappings ✅
...
- [ ]  Create `CrCategoryMappingsSeeder`
- [ ]  Seed for dev/testing school only (or use a config-driven list of school IDs)
- [ ]  Seed entries:
...
- [ ]  Use `updateOrCreate` so seeder is re-runnable without duplicates

---

### 1.3 — period_code Generator 🔒
...
- [ ]  Create `PeriodCodeGenerator` helper (PHP) + matching JS util:
...
- [ ]  Add unit test for the PHP helper (at least 3 date cases across year boundaries)
- [ ]  Use the same JS util inside `SessionContextBar` for client-side `period_code` preview
- [ ]  Document: academic calendar labels ("Week 1", "Week 2" etc.) are display-only and CAN be remapped at will — `period_code` always uses ISO week number for storage integrity

---

### 1.4 — `/api/cr/init-session` Endpoint ✅
...
- [ ]  Create `CrSessionController@initSession`
- [ ]  Validate input: `classroom_id`, `subject_id`, `teacher_id`, `date`, `period_code`, `day_number`, `period_number` (all required)
- [ ]  Upsert `cr_sessions` (create or return existing)
- [ ]  Fetch classroom roster: **`Student::where('classroom_id', $classroom_id)->where('active', true)->get()`** (no junction table — students.classroom_id is the source)
- [ ]  Upsert `cr_student_periods` for each student (insert defaults if not existing)
- [ ]  Upsert `cr_scores` for each (student_period × active_mapping) pair (insert defaults if not existing)
- [ ]  Return: `{ session, students: [{ ...student, period: {...}, scores: [...] }] }`
- [ ]  Wrap entire upsert block in `DB::transaction()`
- [ ]  Add authorization:
  - Teacher: must be assigned to this classroom+subject
  - Admin: allowed read-only access (can call init-session to view, cannot save via batch)

---

### 1.5 — `/api/cr/batch` Endpoint ✅
...
- [ ]  Create `CrSessionController@batchUpdate`
- [ ]  Accept: `{ updates: [{ student_period_id, attendance_status?, attendance_score?, attendance_note?, total_score?, scores?: [{ mapping_id, numeric_value }] }] }`
- [ ]  For each update item:
  - [ ]  Authorize: student_period belongs to the teacher's session
  - [ ]  Validate: `attendance_score` 0..5, `total_score` 0..20, `numeric_value` 0..max\_value from mapping
  - [ ]  Apply absent-lock server-side (if `attendance_status` = absent → force all scores to 0)
  - [ ]  Update `cr_student_periods`
  - [ ]  Update `cr_scores` for each mapping in `scores[]`
- [ ]  Return: `{ updated: [...ids], errors: [...] }`

---

### 1.6 — Attendance=Absent Server-Side Lock ✅
...
- [ ]  In `batchUpdate`: if `attendance_status === 'absent'`:
  - Set `attendance_score = 0`
  - Set all `cr_scores.numeric_value = 0` for this student_period
  - Set `total_score = 0`
  - Set `locked = true`
- [ ]  In `initSession`: if existing record has `locked = true`, skip default overwrite

---

### 1.7 — Unique Constraints (DB Level) ✅

> **Good:** Already defined in schema. Just needs to be verified in migrations and used for upserts.

- [ ]  Confirm `UNIQUE` indexes exist as per cr_recommended_schema.md
- [ ]  Use `updateOrCreate` or `upsert()` instead of `create()` in all init-session logic to prevent race conditions
...
| Phase 1 — Backend          | 🔴 Must   | Yes        | High                 | ⏳ PENDING |
