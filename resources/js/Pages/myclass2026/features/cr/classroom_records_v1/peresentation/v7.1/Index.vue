...
import GroupSetupV2 from './components/group-quiz-v2/setup/GroupSetupV2.vue';
import GroupQuizGeneratorV2 from './components/group-quiz-v2/generator/GroupQuizGeneratorV2.vue';
...
import { useQuizPersistence } from './components/group-quiz-v2/composables/useQuizPersistence';
...
const { restoreSession } = useQuizPersistence();

// Restore saved quiz session on load
restoreSession();
...
<template>
  <div class="v5-container" :class="{ 'has-fixed-description': !ui.isEditMode && presentation.description && presentation.showDescriptionInPresentMode !== false, 'is-focus-mode': ui.isFocusMode }" @contextmenu="handleSlideContextMenu">
    ...
    <!-- Modals & Overlays -->
    <AIPasteDialog />
    <GroupSetupModal v-if="!ui.useGroupQuizV2" />
    <GroupQuizGenerator v-if="!ui.useGroupQuizV2" />
    <GroupSetupV2 v-model="gameStore.isGroupSetupOpen" v-if="ui.useGroupQuizV2" />
    <GroupQuizGeneratorV2 v-model="ui.isGroupQuizGeneratorOpen" v-if="ui.useGroupQuizV2" />
    <LeaderboardOverlay />
    <LiveQuestionPanel />
    <DistributionModal />
